...
namespace InsoniaLiteraria04.ApenasDanceView
{
	...
	public partial class CapituloApenasPage : ContentPage
	{
        ...
        public async void carregarHistoria(int capitulo)
        {
            try
            {
                var listAsync = await serviceCapitulos.carregarCapitulo("CAROLINA", "APENASDANCECOMIGO", "CAPITULO" + capitulo.ToString());
        ...
        {
            Navigation.PushModalAsync(new ComentariosPage("ApenasDance", condicao.ToString()));      
        }
        ...
        async void clkAbrirProximo(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                await Navigation.PushModalAsync(new CapituloApenasPage(condicao + 1));
            }
        ...
        async void salvarViews(int capitulo)
        {
            ...
            try
            {
                var listAsync = await serviceViews.mostrarQuantidadeView("ApenasDance", capitulo.ToString());
                ...
                }, "ApenasDance", capitulo.ToString());

                var listAsyncTotal = await serviceViews.mostrarQuantidadeViewTotal("ApenasDance");
