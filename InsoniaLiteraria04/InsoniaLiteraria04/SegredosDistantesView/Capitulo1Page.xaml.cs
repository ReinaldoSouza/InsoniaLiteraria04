...
namespace InsoniaLiteraria04.SegredosDistantesView
{
    ...
    public partial class Capitulo1Page : ContentPage
    {
        ...
        public Capitulo1Page(int numero)
        {
            ...
            if (numero == Convert.ToInt32(capConstantes.SegDistantes))
            {
                btnCap.Text = "FIM";
            }
            salvarViews(numero);
        ...
        public async void carregarHistoria(int capitulo)
        {
            try
            {
                Historias listAsync = await serviceCapitulos.carregarCapitulo("REINALDO", "DISTANTES", "CAPITULO" + capitulo.ToString());
        ...
        {
            Navigation.PushModalAsync(new ComentariosPage("SegDistantes", condicao.ToString()));
        }
        ...
        async void clkAbrirProximo(object sender, EventArgs e)
        {
            ...
            if (condicao + 1 > Convert.ToInt32(capConstantes.SegDistantes))
            {
                await DisplayAlert("CAPÍTULOS!", "VOCÊ CHEGOU AO ÚLTIMO CAPÍTULO. CONFIRA AGORA SEGREDOS INTERNOS", "OK");
        ...
        async void salvarViews(int capitulo)
        {
            ...
            try
            {
                var listAsync = await serviceViews.mostrarQuantidadeView("SegDistantes", capitulo.ToString());
                ...
                }, "SegDistantes", capitulo.ToString());

                var listAsyncTotal = await serviceViews.mostrarQuantidadeViewTotal("SegDistantes");
