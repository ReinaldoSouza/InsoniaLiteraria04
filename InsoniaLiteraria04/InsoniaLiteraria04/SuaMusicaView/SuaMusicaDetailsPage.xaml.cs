...
namespace InsoniaLiteraria04.SuaMusicaView
{
    ...
    public partial class SuaMusicaDetailsPage : ContentPage
    {
        ...
        {
            var uri = new Uri("https://open.spotify.com/playlist/48on25X86BeMBYTY3e6wBn?si=wIIFfBn_QC217eNW1ly3sA");
        ...
        private async Task<int> MostrarProximoCapitulo(string usuario)
        {
            ...
            try
            {
                var listAsync = await serviceCapitulos.proximoCapitulo(usuario, "SuaMusicaSalvou");
        ...
        private async void clkResetar(object sender, EventArgs e)
        {
            try
            {
                ...
                if (action)
                {
                    var loadingPage = new LoadingPopupPage();
                    await Navigation.PushPopupAsync(loadingPage);
                    await serviceCapitulos.resetarSerie(UserLocalData.userUID, "SuaMusicaSalvou");
                    await Task.Delay(500);
