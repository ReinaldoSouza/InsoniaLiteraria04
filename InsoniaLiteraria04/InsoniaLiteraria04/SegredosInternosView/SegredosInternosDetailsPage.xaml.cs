...
namespace InsoniaLiteraria04.SegredosInternosView
{
    ...
    public partial class SegredosInternosDetailsPage : ContentPage
    {
        DBFireCapitulos serviceCapitulos;
        CapsConstantes capConstantes;
        public SegredosInternosDetailsPage()
        {
            InitializeComponent();
        ...
        private async Task<int> MostrarProximoCapitulo(string usuario)
        {
            ...
            try
            {
                var listAsync = await serviceCapitulos.proximoCapitulo(usuario, "SegInternos");
                ...
                if (listAsync != null)
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 1; i <= listaOrdenada.Count; i++)
                        {
                            ...
                            if (capitulos > Convert.ToInt32(capConstantes.SegInternos))
                            {
        ...
        async void IrParaCapitulo(object sender, EventArgs e)
        {
            ...
            {
                //result = CapsConstantes.SegInternos;
        ...
        private async void clkResetar(object sender, EventArgs e)
        {
            try
            {
                ...
                if (action)
                {
                    var loadingPage = new LoadingPopupPage();
                    await Navigation.PushPopupAsync(loadingPage);
                    await serviceCapitulos.resetarSerie(UserLocalData.userUID, "SegInternos");
                    await Task.Delay(500);
