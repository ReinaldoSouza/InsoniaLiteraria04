...
namespace InsoniaLiteraria04.SegredosInternosView
{
    ...
    public partial class Capitulo1Page : ContentPage
    {
        ...
        public Capitulo1Page(int numero)
        {
            ...
            if (numero == Convert.ToInt32(capConstantes.SegInternos))
            {
                btnCap.Text = "FIM";
            }
            salvarViews(numero);
            mostrarCapituloSalvo("SegInternos", "Capitulo" + numero.ToString());
        ...
        public async void carregarHistoria(int capitulo)
        {
            try
            {
                Historias listAsync = await serviceCapitulos.carregarCapitulo("REINALDO", "INTERNOS", "CAPITULO" + capitulo.ToString());
        ...
        {
            Navigation.PushModalAsync(new ComentariosPage("SegInternos", condicao.ToString()));
        }
        ...
        async void clkAbrirProximo(object sender, EventArgs e)
        {
            ...
            if (condicao + 1 > Convert.ToInt32(capConstantes.SegInternos))
            {
                await DisplayAlert("CAPÍTULOS!", "VOCÊ CHEGOU NO ÚLTIMO CAPÍTULO. AGUARDE PARA MAIS CAPÍTULOS", "OK");
                await Navigation.PushModalAsync(new MenusView.MenuSegredosInternosPage());
            }
            else
            {
        ...
        {
            Navigation.PushModalAsync(new MenusView.MenuSegredosInternosPage());
        }
        ...
        async void LerCapitulo(string capitulo, string status)
        {
            try
            {
                await serviceCapitulos.salvarCapitulo(new CapituloLido()
                {
                    Lido = status,
                    Capitulo = condicao,
                    Historia = "SegInternos",
                    DataGravacao = DateTime.Now.ToString()
                }, UserLocalData.userUID, "SegInternos", "Capitulo" + capitulo);
        ...
        async void salvarViews(int capitulo)
        {
            ...
            try
            {
                var listAsync = await serviceViews.mostrarQuantidadeView("SegInternos", capitulo.ToString());
                ...
                }, "SegInternos", capitulo.ToString());

                var listAsyncTotal = await serviceViews.mostrarQuantidadeViewTotal("SegInternos");
