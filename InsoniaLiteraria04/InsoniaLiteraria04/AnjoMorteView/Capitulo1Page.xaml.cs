...
namespace InsoniaLiteraria04.AnjoMorteView
{
	...
	public partial class Capitulo1Page : ContentPage
	{
        ...
        public async void carregarHistoria(int capitulo)
        {
            try
            {
                var listAsync = await serviceCapitulos.carregarCapitulo("CAROLINA", "ANJOMORTE", "CAPITULO" + capitulo.ToString());
        ...
        {
            Navigation.PushModalAsync(new ComentariosPage("AnjoMorte", condicao.ToString()));
        }
        ...
        async void salvarViews(int capitulo)
        {
            ...
            try
            {
                var listAsync = await serviceViews.mostrarQuantidadeView("AnjoMorte", capitulo.ToString());
                ...
                }, "AnjoMorte", capitulo.ToString());

                var listAsyncTotal = await serviceViews.mostrarQuantidadeViewTotal("AnjoMorte");
